  ...
  const menuItems = [
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Mise à jour', href: '/textes/mise-a-jour', description: 'Collecte et archivage des textes' },
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Mise à jour', href: '/procedures/mise-a-jour', description: 'Collecte des procédures administratives', icon: 'refresh' },
        ...
        <div className="max-w-7xl mx-auto px-4 py-2">
          <div className={`flex items-center justify-between text-sm ${isRTL ? 'flex-row-reverse' : ''}`}>
            <div className={`flex items-center space-x-4 ${isRTL ? 'space-x-reverse' : ''}`}>
              ...
              <span className="text-green-100">{t('algerianRepublicFr')}</span>
            </div>
            <LanguageSelector />
          </div>
        </div>
            ...
            <div className="hidden md:flex items-center flex-1 max-w-md mx-6">
              <div className="relative w-full">
                <input
                  ...
                  className={`border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent w-full py-2 px-4 ${
          ...
          <div className={`flex items-center justify-center space-x-1 ${isRTL ? 'space-x-reverse' : ''}`}>
            {menuItems.map((item) => (
              ...
              >
                <a
                  href={item.href || '#'}
                  className={`flex items-center space-x-2 px-4 py-3 text-sm font-medium transition-colors hover:text-green-600 hover:bg-white rounded-t-lg ${
                    activeDropdown === item.id ? 'text-green-600 bg-white shadow-sm' : 'text-gray-700'
                      ...
                      {item.items?.map((subItem, index) => (
                        <a
                          ...
                          className={`block px-4 py-3 rounded-lg hover:bg-gray-50 transition-colors group ${
                            subItem.highlight ? 'bg-green-50 border border-green-200' : ''
                          }`}
                        >
                          <div className={`flex items-center space-x-3 ${isRTL ? 'space-x-reverse' : ''}`}>
                            {subItem.icon === 'plus' && (
                              <Plus className={`h-4 w-4 ${subItem.highlight ? 'text-green-600' : 'text-gray-400'}`} />
                            )}
                            <div className={`flex-1 ${isRTL ? 'text-right' : 'text-left'}`}>
                              <div className={`font-medium text-sm ${
                                subItem.highlight ? 'text-green-700' : 'text-gray-900 group-hover:text-green-600'
                              }`}>
                                {subItem.label}
                              </div>
                              <div className={`text-xs mt-1 ${
                                subItem.highlight ? 'text-green-600' : 'text-gray-500'
                              }`}>
                                {subItem.description}
                              </div>
                            </div>
