...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const legalDomains = [
    { name: t('laborLaw'), count: 45, trend: '+5', color: 'bg-green-600' },
        ...
        <div className="bg-gradient-to-r from-green-600 to-green-700 rounded-2xl p-8 text-white">
          <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
            <div className={isRTL ? 'text-right' : 'text-left'}>
              ...
              <div className={`flex items-center space-x-4 text-green-100 ${isRTL ? 'space-x-reverse' : ''}`}>
          ...
          {stats.map((stat, index) => (
            <div key={index} className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition-all duration-200">
              <div className={`flex items-center justify-between mb-4 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`flex items-center space-x-1 text-green-600 text-sm font-medium ${isRTL ? 'space-x-reverse' : ''}`}>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <div className={`flex items-center justify-between mb-6 ${isRTL ? 'flex-row-reverse' : ''}`}>
                ...
                <button className={`text-green-600 hover:text-green-700 text-sm font-medium flex items-center space-x-1 ${isRTL ? 'space-x-reverse' : ''}`}>
                  <span>Voir tout</span>
                ...
                {recentDocuments.map((doc, index) => (
                  <div key={index} className={`flex items-start space-x-4 p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors cursor-pointer ${isRTL ? 'space-x-reverse' : ''}`}>
                    <div className={`
                      ...
                      ${doc.importance === 'high' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}
                    `}>
                    ...
                    <span className={`
                      ...
                      ${doc.status === 'Nouveau' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}
                    `}>
                      {doc.status}
                    </span>
                  </div>
                ...
                {legalDomains.map((domain, index) => (
                  <div key={index} className="p-4 bg-gray-50 rounded-xl">
                    <div className={`flex items-center justify-between mb-2 ${isRTL ? 'flex-row-reverse' : ''}`}>
                      ...
                      <span className="text-xs text-green-600 font-medium">{domain.trend}</span>
                    </div>
